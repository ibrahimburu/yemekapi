...
const postdetails = async (req, res) => {
    return new Promise(async resolve => {
        ...
        const sqlForRecord = `SELECT * FROM recorded_posts where user_id = ? AND post_id = ?`;
        ...
        try {
            ...
            if (post == null) {
                ...
                if (user == "") {
                    ...
                    const alreadyRecorded = await dbhelper(sqlForRecord, [req?.id, post[0]?.id]);
                    if (alreadyLiked != "") {
                        already_liked = true;
                    }
                    if (alreadyRecorded != "") {
                    ...
                    const result = {
                        ...
                        recorded_by_user:already_recorded
                }
                    const message = {
